...
class Page extends MY_Admin_Controller {
    ...
    public function index()
    {
        ...
        $list = $this->Page_model->fetch_page($page, $pagesize, $arrWhere,'*',$strOrder);
        ...
        //分页
        $pagecfg = array();
        $pagecfg['base_url']     = _create_url(ADMIN_SITE_URL.'/Page', $arrParam);
    ...
    public function add()
    {
        ...
        $this->load->model('Page_Model');
        $arrPlace = $this->Page_Model->get_list();
    
        if(!empty($id))
        {
            $info = $this->Page_model->get_by_id($id);
            $place = $this->Page_Model->get_by_id($info['id'],'title');
    ...
    //var_dump($info);die;
        $this->load->view('admin/page_add', $result);
    }
...
//接收图片必须用这个，数据库里才会有图片名字                 
             ...
             //        $data['id'] = $id;
                ...
                redirect(ADMIN_SITE_URL.'/page');
                exit;
            }
            else
            {//echo base_url('/admin/link1');die;
                redirect(ADMIN_SITE_URL.'/page/add');
            }
    ...
    function del(){
        ...
        {   
            ...
            foreach($id as $k=>$v){
                ...
                $data['status'] = -1;
                // 调用修改状态方法
                $this->Page_model->update_by_id($v,$data);
            }
        ...
        else
        {
            ...
            $data['status'] = -1;
        // 调用修改状态方法
            $this->Page_model->update_by_id($id,$data);
            ;
        }
