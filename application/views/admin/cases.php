...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base">
        ...
        <li><a href="JavaScript:void(0);" class="current"><span>案例列表<?php echo lang('adv_manage');?></span></a></li>
        <li><a href="<?php echo ADMIN_SITE_URL.'/cases/add';?>"><span >添加案例</span></a></li>
      </ul>
    </div>
  </div>
  ...
  <form method="post" id="store_form" action="<?php echo ADMIN_SITE_URL.'/cases/del'?>">
    ...
    <table class="table tb-type2">
      ...
      <tbody>
        ...
        <tr class="hover">
          ...
          <td><?php if(isset($class_list[$v['class_id']])) echo $class_list[$v['class_id']]; ?></td>
          <td><?php if($v['status'] ==1 ) echo '显示'; elseif ($v['status'] ==2) echo '不显示';?>
          </td>
          <td class="w72 align-center">
          <a href="<?php echo ADMIN_SITE_URL.'/cases/add?id='.$v['id'];?>">编辑</a>&nbsp;|&nbsp;
          <a href="<?php echo ADMIN_SITE_URL.'/cases/del?id='.$v['id'];?>">删除</a></td>
        </tr>
        <?php } ?>
        <?php }else { ?>
        <tr class="no_data">
