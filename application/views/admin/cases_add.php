  ...
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base">
        ...
        <li><a href="<?php echo ADMIN_SITE_URL.'/cases/';?>" ><span>案例列表<?php echo lang('adv_manage');?></span></a></li>
        <li><a href="<?php echo ADMIN_SITE_URL.'/cases/add';?>" class="current"><span>添加案例</span></a></li>
      </ul>
    </div>
  </div>
  <div class="fixed-empty"></div>
 <form id="adv_form" method="post" action="<?php echo ADMIN_SITE_URL.'/cases/save'?>">
    ...
    <!--<table class="table tb-type2 nobdb">-->
      <tbody>
        <tr class="noborder">
          <td colspan="2" class="required"><label class="validation" for="cases_title">标题:</label></td>
        </tr>
        <tr class="noborder">
          <td class="vatop rowform"><input type="text" value="<?php if (isset($_GET['id'])){ echo $info['title'];}?>" name="title" id="cases_title" class="txt"></td>
        </tr>
        ...
        <tr>
          <td colspan="2" class="required"><label for="cases_pic">图片:</label></td>
        </tr>
        ...
        <tr>
          <td colspan="2" class="required"><label for="cate_id">分类名:</label></td>
        </tr>
        <tr class="noborder">
          <td class="vatop rowform"><select name="class_id" >
              <option value="">请选择...</option>
              <?php if(!empty($class_list) && is_array($class_list)): ?>
                <?php foreach($class_list as $k => $v): ?>
                  <option  <?php if(!empty($info) && $info['class_id'] == $v['id']) echo 'selected="selected"';?> value="<?php echo $v['id'];?>"><?php echo $v['name'];?></option>
                <?php endforeach; ?>
              <?php endif; ?>
            </select></td>
          <td class="vatop tips"></td>
        ...
        <tr class="noborder">
          <td colspan="2" class="required"><label for="cases_url">网址:</label></td>
        </tr>
        <tr class="noborder">
          <td class="vatop rowform"><input type="text" value="<?php if (isset($_GET['id'])){ echo $info['url'];}else{echo 'http://';}?>" name="url" id="cases_url" class="txt"></td>
        </tr>
        
        
        <tr>
            <td colspan="2" class="required"><label for="cases_sort">状态:</label></td>
        </tr>
        <tr class="noborder">
